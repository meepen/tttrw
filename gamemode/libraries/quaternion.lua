...
local halfDegToRad = 0.5 * math.rad(1)
local _forward = VECTOR.Forward
local _up = VECTOR.Up
local _next = {2, 3, 1}


local QUATERNION = FindMetaTable "Quaternion" or {
	MetaName = "Quaternion"
}
...
local function UnclampedSlerp(from, to, t)		
	...
	if cosAngle < 0.95 then    
		...
		local quat = Quaternion(from.x * t1 + to.x * t2, from.y * t1 + to.y * t2, from.z * t1 + to.z * t2, from.w * t1 + to.w * t2)
		return quat
	else    
...
function QUATERNION:ToAngleAxis()		
	...
	if Approximately(angle, 0) then
		return angle * 57.29578, Vector(1, 0, 0)
	end
	
	local div = 1 / sqrt(1 - sqrt(self.w))
	return angle * 57.29578, Vector(q.x * div, q.y * div, q.z * div)
end
