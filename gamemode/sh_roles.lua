...
function GM:TTTPrepareRoles(Team, Role)
	local sees_innocents = {}

	hook.Run("TTTRoleSeesRole", "innocent", sees_innocents)

	local sees_traitors = {}

	hook.Run("TTTRoleSeesRole", "traitor", sees_traitors)

	Team "innocent"
		:SeenBy {unpack(sees_innocents)}
		:SetColor(19, 130, 77)
		:SetDeathIcon "materials/tttrw/roles/innocent.png"

	Team "traitor"
		:SeenBy {"traitor", unpack(sees_traitors)}
		:SetColor(136, 21, 22)
		:SetDefaultCredits(2)
