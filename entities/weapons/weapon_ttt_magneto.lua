...
SWEP.Ortho = {-1, 0.5, angle = Angle(-90, 50, 0), size = 0.9}
...
function SWEP:PinRagdoll()
	...
	if (not self:GetOwner():GetRoleData().Evil and not pin_rag_inno:GetBool()) then
		return
	end
...
function SWEP:SetupDataTables()
	...
	self:NetVar("CanRagPin", "Bool", false)
	self:NetVar("CarriedRag", "Entity", NULL)
end

function SWEP:Initialize()
	if SERVER then
		self:SetCanRagPin(pin_rag:GetBool())
		self:SetCarriedRag(NULL)
	end
...
function SWEP:DrawHUD()
	...
	if (self:GetCanRagPin() and IsValid(self:GetCarriedRag())) then
		...
		if (not client:IsSpec() and client:GetRoleData().Evil) then
